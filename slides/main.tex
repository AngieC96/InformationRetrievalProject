...
\begin{frame}{Index}
    ...
    We have an index [\texttt{\alert{class Index}}] that is a mixture of:
        \begin{itemize}
            \item[\alert{$\bullet$}] {\makebox[3.3cm][l]{An inverted index} $\longrightarrow$ \quad the basic index}
            \item[\alert{$\bullet$}] {\makebox[3.3cm][l]{A positional index} $\longrightarrow$ \quad for phrase queries}
            \item[\alert{$\bullet$}] {\makebox[3.3cm][l]{A permuterm index} $\longrightarrow$ \quad for wildcards queries}
        \end{itemize}
